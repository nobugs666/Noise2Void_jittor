...
if __name__ == '__main__':
    ...
    parser.add_argument('--gaussian_noise_level', type=str, default='30') # 10，30，50，70
    ...
    jt.misc.set_global_seed(opt.seed)
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            ...
            checkpoint = jt.load(opt.resume) # 加载模型
            opt.start_epoch = checkpoint["epoch"] + 1   # 从最后一个检查点获取epoch加1，确定从哪一个epoch开始继续训练
            model.load_state_dict(checkpoint["model"]) # 将模型状态字典加载到checkpoint中
        else:
            print("=> no checkpoint found at '{}'".format(opt.resume))

    # criterion = nn.MSELoss()

    optimizer = jt.optim.Adam(model.parameters(), lr=opt.lr)
    ...
    dataset = Dataset(opt.images_dir, opt.patch_size, opt.gaussian_noise_level, opt.downsampling_factor, opt.jpeg_quality, opt.is_gray,).set_attrs(batch_size=opt.batch_size,
                            shuffle=True,
    ...
    eval_dataset = EvalDataset(opt.clean_valid_dir, opt.gaussian_noise_level, opt.downsampling_factor, opt.jpeg_quality, opt.is_gray).set_attrs(batch_size=1)
    ...
    for epoch in range(opt.start_epoch, opt.num_epochs):
        ...
        if (epoch + 1) % opt.epoch_save_num == 0:
            ...
            # 原错误代码
            # state = {"epoch": epoch, "model": model}  # 错误：保存整个模型实例
            ...
            state = {
                "epoch": epoch,
                "model": model.state_dict()  # 只保存模型参数字典
            }
            jt.save(state, os.path.join(opt.outputs_dir,
                                        '{}_epoch_{}_{}.pth'.format(opt.arch, epoch, opt.gaussian_noise_level)))
        ...
        # 原代码
        jt.save(best_weights, os.path.join(opt.outputs_dir, best_weight_path))  # 若best_weights是state_dict则无需修改
    writer.close()
...
def calc_psnr(img1, img2):
    if isinstance(img1, jt.Var):
        return 10. * jt.log10(1. / jt.mean((img1 - img2) ** 2))
    else:
...
def preprocess(img, device, image_mode='RGB'):
    if image_mode == 'RGB':
        ...
        ycbcr /= 255.
        ycbcr = jt.array(ycbcr).unsqueeze(0)  # numpy -> cpu tensor -> GPU tensor
        ...
        y /= 255.
        y = jt.array(y)  # numpy -> cpu tensor -> GPU tensor
...
def viz_layer(layer, n_filters=4):
    ...
    # 转换为numpy数组（Jittor张量直接用.numpy()）
    layer_np = layer.numpy() if isinstance(layer, jt.Var) else layer
    min_val = np.min(layer_np)
    max_val = np.max(layer_np)

    for index, filter in enumerate(layer_np):
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        plt.subplot(8, 8, index + 1)
        # 归一化到[0,1]
        filter_norm = (filter - min_val) / (max_val - min_val)
        plt.imshow(filter_norm[0, :, :], cmap='gray')
        plt.axis('off')
    plt.show()
...
def viz_layer2(layer, n_filters=4):
    plt.figure(figsize=(4, 3.5))
    # 转换为numpy数组（如果是Jittor张量）
    layer_np = layer.numpy() if isinstance(layer, jt.Var) else layer
    min_val = np.min(layer_np)
    max_val = np.max(layer_np)

    for index in range(n_filters):
        filter = layer_np[:, :, index]
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        plt.subplot(8, 8, index + 1)
